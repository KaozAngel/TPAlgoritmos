#ifndef _LISTA_DOB_PF_
#define	_LISTA_DOB_PF_
...
bool ListaDobPF::Insertar_Inicio(char* dato)
{
...
bool ListaDobPF::Insertar_Final(char* dato)
{
	if (inicio == nullptr)
...
bool ListaDobPF::Insertar_Posicion(char* dato, int pos) // Restarle -1 al pos.
{
...
bool ListaDobPF::Eliminar_Posicion(int pos)
{
	if (pos <1 || pos>numElementos)
		return false;
...
bool ListaDobPF::Eliminar_Inicio() {
	if (numElementos == 1) {
		inicio = nullptr;
		free(inicio);
		return true;
...
bool ListaDobPF::Eliminar_Final() {
	if (numElementos == 1) {
		return Eliminar_Inicio();
	}
	CNodo*aux = rear;
...
void ListaDobPF::Draw_List(Graphics^g) {
